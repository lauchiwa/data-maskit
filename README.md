...
- **无缝接入 Web 端大模型**：专为 **ChatGPT、Claude、Kimi、豆包、通义千问** 等网页版 AI 打造的 Chrome/Edge MV3 扩展，预置 18+ 常见 AI 站点一键授权；
- **全链路双通道拦截（Fetch + XHR 双引擎）**：除常规 Fetch 外，深度拦截底层 `XMLHttpRequest` 的请求与流式响应（XHR 响应还原目前覆盖 DeepSeek 网页版），其余站点保持原生行为零干预；
- **文档与附件直传脱敏**：支持 Web 端上传 Word（`.docx` / `.doc`）、Excel（`.xlsx` / `.xls`）、PowerPoint（`.pptx`）等文件时在本地自动解析并打码出网；
- **本地拦截打码 + 网页打字机流式还原**：页面发出的 Prompt 与附件在本地引擎打码后出网，回复在网页界面流式还原，支持多轮会话一致性保持。
...
> 💡 **状态与排查要点**：
> - **文件与附件脱敏**：扩展已全面支持网页端附件直传与 Office 文档脱敏（`.docx` / `.xlsx` / `.pptx` 及转码 `.doc` / `.xls`），无需人工预先编辑；
